...
class UserEventActionView(APIView):
    ...
    def post(self, request, slug):
        ...
        # 1. Create the QR Ticket
        attendance, created = EventAttendance.objects.get_or_create(user=user, event=event)
        
        # 2. SYNC FIX: Also add to the simple attendees list 
        # (This ensures the main Events List page knows you are going)
        event.attendees.add(user) 
    ...
    def delete(self, request, slug):
        ...
        try:
            ...
            # 2. SYNC FIX: Remove from the simple attendees list too
            event.attendees.remove(user)
            
        except EventAttendance.DoesNotExist:
            return Response({"message": "Not registered"}, status=status.HTTP_404_NOT_FOUND)

        return Response({"success": True, "message": "Unregistered successfully"}, status=status.HTTP_200_OK)
...
class AdminScanView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"error": "Invalid Ticket - Not Found"}, status=status.HTTP_404_NOT_FOUND)
...
class DashboardView(APIView): 
    ...
    def get(self, request):
        ...
        checklist = {
            "profile_complete": all([user.major, user.class_standing, user.first_name, user.last_name]),
