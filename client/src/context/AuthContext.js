...
export const AuthProvider = ({ children }) => {
    ...
    const loginUser = async (email, password) => {
        try {
            const { data, status } = await axios.post(`${baseUrl}/token/`, { email, password });
    ...
    const registerUser = async (first_name, last_name, major, class_standing, email, password, password2) => {
        try {
            const { status } = await axios.post(`${baseUrl}/register/`, {
                first_name, last_name, major, class_standing, email, password, password2
            });
    ...
    const verifyEmail = async (token, setVerifiedUserEmail) => { 
        ...
        try {
            const { data, status } = await axios.post(`${baseUrl}/token/verify/`, { token });
    ...
    const passwordResetRequest = async (email) => {
        try {
            const response = await axios.post(`${baseUrl}/password-reset-request/`, { email });
    ...
    const passwordReset = async (password, password2) => {
        try {
            ...
            const { status } = await axios.put(`${baseUrl}/password-reset/`, { email, password, password2 });
