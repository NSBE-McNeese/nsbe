...
import {
  Container, Grid, Paper, Typography, Box, Button, Divider, Chip, LinearProgress,
  Alert
} from "@mui/material";
...
const EventDetail = () => {
  ...
  const onToggleRegister = async () => {
    // Don't allow changes if ticket is approved
    if (isApproved) {
    ...
    try {
      ...
      // Re-check status immediately after action
      await checkAttendanceStatus(event_slug);
    } catch (error) {
    ...
    <Box sx={{ bgcolor: "#f8f9fa", minHeight: "100vh", pb: 8 }}>
      ...
      <Box 
        sx={{ 
          height: 350, 
          bgcolor: "#00205B", 
          backgroundImage: eventDetail.image ? `url(http://127.0.0.1:8000${eventDetail.image})` : "linear-gradient(135deg, #005090 0%, #003366 100%)",
      ...
      >
        <Box sx={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.8) 100%)" }} />
        ...
        <Container maxWidth="lg" sx={{ position: "relative", zIndex: 2, pb: 6 }}>
          <Button 
            ...
            sx={{ color: "rgba(255,255,255,0.8)", mb: 2, fontWeight: "bold", textTransform: "none", "&:hover": { color: "white" } }}
          >
            Back to Events
          </Button>
          
          <Typography variant="h2" fontWeight="800" color="white" sx={{ textShadow: "0px 4px 10px rgba(0,0,0,0.3)", fontSize: { xs: "2rem", md: "3rem" } }}>
            {eventDetail.title}
          </Typography>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={7} lg={8}>
            <Paper elevation={3} sx={{ p: 4, borderRadius: 3, mb: 4 }}>
              <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: "#00205B", borderBottom: "3px solid #FFC72C", display: "inline-block", pb: 1, mb: 3 }}>
                Event Details
              </Typography>
              
              <Typography variant="body1" sx={{ lineHeight: 1.8, fontSize: "1.1rem", color: "#444", whiteSpace: "pre-line" }}>
              ...
              <Box sx={{ 
                ...
                <Typography variant="h6" fontWeight="bold">
                  {isApproved ? "Ticket Approved!" : 
                   isRegistered ? "Registration Confirmed" : 
                   "Event Info"}
                </Typography>
              ...
              <Box sx={{ p: 3 }}>
                ...
                {isRegistered && qrToken && (
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 3 }}>
                    <Box sx={{ p: 2, bgcolor: 'white', border: isApproved ? '2px solid #2e7d32' : '2px dashed #ccc', borderRadius: 2 }}>
                       <QRCode 
                          value={qrToken}
                          size={150}
                ...
                {/* Points Badge */}
                <Box sx={{ bgcolor: "#fff8e1", border: "1px solid #FFC72C", borderRadius: 2, p: 2, mb: 3, display: "flex", alignItems: "center", gap: 1.5 }}>
                ...
                {!isPast ? (
                  ...
                  <Button variant="contained" disabled fullWidth>Event Ended</Button>
                )}
