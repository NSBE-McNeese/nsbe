          ...
          {currentEventStatus &&
            (userIsRegistered ? (
              <Button
                variant="outlined"
                color="primary"
                onClick={() => handleUnregistration(event_slug)}
              >
                Unregister
              </Button>
            ) : (
              <Button
                variant="contained"
                color="primary"
                onClick={() => handleRegistration(event_slug)}
              >
                Register
              </Button>
            ))}
        ...
        <Grid container spacing={2} padding={2}>
          <Grid item xs={12}>
            <Typography variant="body2">
              Start: {new Date(eventDetail.start_time).toLocaleString()}
            </Typography>
            <Typography variant="body2">
              Start: {new Date(eventDetail.end_time).toLocaleString()}
            </Typography>
            <MapComponent />
            <Paper
              elevation={4}
