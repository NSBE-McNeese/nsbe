...
const ProfilePage = () => {
  ...
  const api = useAxios(); // This is the instance we will use
  ...
  const handleUpdate = async (field, newValue) => {
    try {
      const response = await api.patch("/profile/update/", { [field]: newValue });
      if (response.status === 200) {
  ...
  const handleDeleteAccount = async () => {
    if (!deletePassword) {
      toast.warning("Enter your password to confirm.");
      return;
    }
    try {
      await api.delete("/profile/update/", { data: { password: deletePassword } });
  ...
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
      <Paper elevation={4} sx={{ borderRadius: 4, overflow: "hidden", bgcolor: "#fff" }}>
        <Box
          sx={{ height: 140, background: "linear-gradient(135deg, #00205B 0%, #005090 100%)" }}
        />
        ...
        <Box sx={{ p: { xs: 3, md: 5 } }}>
          ...
          <EditField
            ...
            name="class_standing"
            value={profile?.class_standing}
            options={choices.class_standings}
            icon={<BadgeIcon />}
            onSave={handleUpdate}
          />
          <EditField
          ...
          <Box
            sx={{ mt: 6, bgcolor: "#fff5f5", p: 3, borderRadius: 2, border: "1px dashed #ef5350" }}
          >
      ...
      <Dialog open={openDelete} onClose={() => setOpenDelete(false)}>
        <DialogTitle sx={{ color: "#d32f2f", fontWeight: "bold" }}>Delete account?</DialogTitle>
        <DialogContent>
